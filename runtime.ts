...
export class BaseAPI {
    ...
    private async createFetchParams(context: RequestOpts, initOverrides?: RequestInit | InitOverrideFunction) {
        ...
        // Add apiKey to headers if present
        if (this.configuration.apiKey) {
            const apiKey = await this.configuration.apiKey('api_key');
            if (apiKey) {
                headers['X-API-KEY'] = apiKey; // Change header name if needed
            }
        }

        // Add accessToken to headers if present
        if (this.configuration.accessToken) {
            const accessToken = await this.configuration.accessToken();
            if (accessToken) {
